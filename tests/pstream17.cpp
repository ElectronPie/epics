...
void do_pistream_work(eps::pistream<std::stringstream> pistream, std::vector<bool>::reference result)
{
    ...
    result = std::adjacent_find(readings.cbegin(), readings.cend(), std::not_equal_to<data_type>{}) == readings.cend();
}
...
void do_pstream_work(
    data_type n, eps::pstream<std::stringstream, std::stringstream> pstream, std::vector<bool>::reference result
)
{
    ...
    result = std::all_of(readings.cbegin(), readings.cend(), std::bind(std::equal_to<data_type>{}, _1, n));
}
...
TEST_CASE("testing pstream")
{
...

    {
        ...
        go = false;
        threads.clear();
        CHECK(std::all_of(results.cbegin(), results.cend(), [](bool x) constexpr -> bool { return x; }));
    }

    {
        ...
        for (unsigned int i = 0; i < num_threads; ++i)
        {
            ...
            results[i] = std::adjacent_find(readings.cbegin(), readings.cend(), std::not_equal_to<data_type>{})
                         == readings.cend();
        }
        CHECK(std::all_of(results.cbegin(), results.cend(), [](bool x) constexpr -> bool { return x; }));
        CHECK(ss.eof());
...

    {
        ...
        for (unsigned int i = 0; i < num_threads; ++i)
        {
            threads.emplace_back(
                do_pstream_work, i, eps::pstream<std::stringstream, std::stringstream>{ss, ss}, results[i]
            );
        }
        go = true;
        for (std::thread& t : threads)
